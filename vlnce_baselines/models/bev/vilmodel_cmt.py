...
class BertOutAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        self.kv_scale = config.kv_scale if 'kv_scale' in config.to_dict() else 1.0
        attention_drop_prob = config.TM_cross_attention_probs if 'TM_cross_attention_probs' in config.to_dict() else config.attention_probs_dropout_prob
        self.dropout = nn.Dropout(p=attention_drop_prob)
    ...
    def forward(self, hidden_states, context, attention_mask=None):
        ...
        if attention_mask is not None:
            attention_scores = attention_scores * self.kv_scale
            attention_scores = attention_scores + attention_mask
...
class CrossmodalEncoder(nn.Module):
    def __init__(self, config, num_layer=None):
        super().__init__()
        self.num_x_layers = num_layer if num_layer is not None else  config.num_x_layers
...
class LocalBEVEncoder(nn.Module):
    ...
    def forward(
        ...
        bev_embeds = self.encoder(txt_embeds, txt_masks, bev_embeds, bev_masks)
...
class TokenErrorPosition(nn.Module):
...
class TrajectoryMatcher(nn.Module):
    def __init__(self, config, num_layer) -> None:
        ...
        self.trajectory_set_cls_token = nn.Parameter(torch.zeros(1, 1, config.hidden_size))
        config.update({'TM_cross_attention_probs': 0.45})
        config.update({'kv_scale': 0.9})
        self.encoder = CrossmodalEncoder(config, num_layer=num_layer)
        self.predictor = ClsPrediction(input_size=768, hidden_size=768)

        self.predictor_token_error_position = TokenErrorPosition()

    def forward(self, txt_embeds, txt_masks, img_embeds, img_masks, graph_sprels=None):
        # add the cls token to the trajectory set
        # add another item to the mask as True
        find_last_idx = (img_masks == False).nonzero(as_tuple=True)[1][0].item()
        img_masks[:, find_last_idx] = True

        # prepend the cls token to the traj set
        # note : img_embes in pos 15 will never be set
        img_embeds = torch.roll(img_embeds, shifts=1, dims=1) # shift every item to the right
        # add cls tokens
        img_embeds[:, 0, :] = self.trajectory_set_cls_token

        
        pos_encoder = SinCosPositionalEncoding(d_model=768, max_len=16).to(txt_embeds.device)
        img_embeds = pos_encoder(img_embeds.permute(1,0,2)).permute(1, 0, 2)

        # Trajectory set as query, Instruction as KV
        hist_emb_X_instr = self.encoder(txt_embeds, txt_masks, img_embeds, img_masks, graph_sprels=None)
        cls_emb = hist_emb_X_instr[:, 0, :] # take the cls token
        return self.predictor(cls_emb), self.predictor_token_error_position(cls_emb)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.trajectory_matching_original_bev = TrajectoryMatcher(config, num_layer=4)
    ...
    def forward_trajectory_matcher(self, all_txt_embeds, all_txt_masks, all_history_embeds, all_history_mask):
        # here ,history are just the visual embedding 
        return self.trajectory_matching_original_bev(all_txt_embeds, all_txt_masks, all_history_embeds, all_history_mask)
    ...
    def forward_navigation(
        ...
        # global branch
        gmap_embeds = (
            gmap_img_fts  + self.global_encoder.gmap_step_embeddings(gmap_step_ids) + self.global_encoder.gmap_pos_embeddings(gmap_pos_fts)
        ...
        fuse_weights = torch.sigmoid(
            self.sap_fuse_linear(torch.cat([gmap_embeds[:, 0], bev_embeds[:, CENTER]], 1))
        )
        ...
        progress_hat = None
        hist_x_att_sub_scores = None
        history_embeds_current_node = None

        return outs, similarity_score, None, hist_x_att_sub_scores, progress_hat, history_embeds_current_node
