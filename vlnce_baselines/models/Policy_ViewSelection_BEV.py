...
class BEV(Net):
    def __init__(
        ...
        self.grid_pool_depth = nn.Sequential(
            nn.AdaptiveAvgPool2d((14, 14)), nn.Flatten(start_dim=1)
        )
        ...
        pano_angle_rad_c = (
            (1 - self.pano_img_idxes / 12) * 2 * math.pi
        )  # clockwise  array([360, 330, 300, 270, 240, 210, 180, 150, 120,  90,  60, 30])
        self.pano_angle_fts = angle_feature_torch(
            torch.from_numpy(pano_angle_rad_c)
        )  # clockwise
    ...
    def forward(
        ...
        if mode == "language":
        ...
        elif mode == "trajectory_matcher":
            return self.vln_bert.forward_trajectory_matcher(txt_embeds, txt_masks, hist_embeds, hist_mask)
            ...
            rgb_feats = torch.cat(
                [
                    rgb_embeds_reshape[:, 0:1, :],
                    torch.flip(rgb_embeds_reshape[:, 1:, :], [1]),
                ],
                dim=1,
            )
            ...
            depth_feats = torch.cat(
                [
                    depth_embeds_reshape[:, 0:1, :],
                    torch.flip(depth_embeds_reshape[:, 1:, :], [1]),
                ],
                dim=1,
            )
