...
from clip import simple_tokenizer as clip_tokenizer
...
class SceneAwareness(object):
    def __init__(self, sub_instruction_index, alignment_score, progress_estimation):
        '''
        class representing data for a specific time step
        '''
        self.rooms: List[str] = []
        self.objects: List[str] = []
        self.sub_instruction_index : int = sub_instruction_index
        self.alignment_score : int = alignment_score
        self.previous_sub_scene_awareness : SceneAwareness = None
        self.progress_estimation : float = progress_estimation
    ...
    def add_alignment_score(self, score):
...
class GraphMap(object):
    def __init__(self, has_real_pos, loc_noise, merge_ghost, ghost_aug):
        ...
        # LOCALIZATION WITH TEXT
        self.sub_instruction_ids_to_tokens = None #sub 0 -> tokens, sub 1 -> tokens
        self.instruction_ids = None
        self.instruction_ids_mask = None

        self.step_id_to_scene_awareness : Dict[int, SceneAwareness] = {}
        self.clip_tokenizer = clip_tokenizer.SimpleTokenizer()
        self.alignment_method_solver = None


        self.ep_id = None
        self.episode_id_to_history_index = None
    
    def set_ep_id(self, ep_id):
        self.ep_id = ep_id

    def set_index_for_history_information(self, episode_id_to_history_index):
        ''' map the episode id to the index of the history, so
            when an episode is terminated we know exactly which index of the ALL_hsitory_embes we have to take
        '''
        self.episode_id_to_history_index = episode_id_to_history_index        
