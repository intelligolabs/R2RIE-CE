...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _eval_checkpoint(
        ...
        while envs.num_envs > 0 and len(stats_episodes) < episodes_to_eval:
            ...
            for i in range(envs.num_envs):
                if (
                    next_episodes[i].episode_id in stats_episodes
                ):  # 出现了重复的ep，表示这个模拟器中的episode已经全部过了一遍
    ...
    def eval(self):
        ...
        with TensorboardWriter(
            ...
            if os.path.isfile(self.config.EVAL.CKPT_PATH_DIR):
                ...
                # else:
                #     ckpt_idx = 0
                logger.info(f"=======current_ckpt: {self.config.EVAL.CKPT_PATH_DIR}=======")
                ...
                while True:
                    ...
                    while current_ckpt is None:
                        ...
                        if "_tm" not in current_ckpt.split("/")[-1]:
                            prev_ckpt_ind += 1
                            current_ckpt = None # we olny want to load the ckpt with _tm checkpoints
                        time.sleep(2)  # sleep for 2 secs before polling again
                        

                    if self.local_rank < 1:
                        logger.info(f"=======current_ckpt: {current_ckpt}[ckpt index: {self.get_ckpt_id(current_ckpt)}]=======")
                    prev_ckpt_ind += 1
                    self._eval_checkpoint(
    ...
    def get_ckpt_id(self, ckpt_path):
        ...
        ckpt_id = ckpt_path.split(".")[1].replace("iter", "").replace("_tm", "")
