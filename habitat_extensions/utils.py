...
def append_text_to_image(image: np.ndarray, text: str, score_and_associated_sub=None, progress_hat=None):
    ...
    Args:
        ...
        score_and_associated_sub : zip of score and associated sub instruction text - zip object,
        progress_hat : progress  of the agent - float
    Returns:
    ...
    if progress_hat is None:
        progress_hat = 0
    y = 0
    if score_and_associated_sub is not None:

        cmap = plt.get_cmap('viridis')
        scores = score_and_associated_sub['score']
        normalized_scores = np.array(scores) / max(scores)
        colors = [cmap(score) for score in normalized_scores]

        # white for all sub. Red for the most attenden, orange fro the second and yellowe for the third 
        rgb_colors = [(int(255), int( 255), int(255)) for color in colors]
        scores = torch.tensor(scores)
        _, topk = torch.topk(scores, 3)
        topk = topk.tolist()
        rgb_colors[topk[0]] = (255, 0, 127) # color for red 
        rgb_colors[topk[1]] = (255, 165, 0) # rgb color for orange 
        rgb_colors[topk[2]] = (255, 255, 0)  # rgb  yellow 

        cv2.putText(
                    blank_image,
                    f"Progress:{progress_hat:.2f}",
                    (w-80, 0+30),
                    font,
                    font_size,
                    (255, 255, 255),
        ...
        for ix, sub in enumerate(score_and_associated_sub['sub_txt']):
            wrapped_text = textwrap.wrap(sub, width=int(w / char_size[0]))
            for line in wrapped_text:
...
def planner_video_frame(
    ...
    # unique_filename = str(vis_info['step_k'])
    # Image.fromarray(rgb).save(f"/home/ftaioli/projects/bevbert_uncertainty/images_cube/{unique_filename}.jpg")
    return frame
...
def navigator_video_frame(
    ...
    if vis_info is None:
        score_and_associated_sub = None
        progress_hat = None
    else : 
        score_and_associated_sub = vis_info['score_and_associated_sub']
        progress_hat = vis_info['progress_hat']

    step = "" if vis_info is None else vis_info['step_k']
    frame = add_id_on_img(frame, f"Step:{step}")
    frame = append_text_to_image(frame, observations["instruction"]["text"], score_and_associated_sub, progress_hat)
    return frame
