...
@attr.s(auto_attribs=True)
class TokenSwappedData:
    text : str = attr.ib(default='') # text representing the token (tokenizer.decode([token]))
    token_id : int = attr.ib(default=0) # id of the token
    token_id_position : int = attr.ib(default=-1) # token position inside the instruction

@attr.s(auto_attribs=True)
class ErrorInformationData:
    episode_contains_error : bool = attr.ib(default=False) # if the episode contains or an error generated by us
    error_type : int = attr.ib(default=-1) # direction token, scene_awareness tokens or both. If both, error are in the same sub-instruction 
    token_swapped : Optional[List[TokenSwappedData]] = attr.ib(default=-1) # the token(s) that are swapped
...
class VLNExtendedEpisode(VLNEpisode):
    ...
    error_information : ErrorInformationData  = attr.ib(default=None)
    
    old_episode_id : Optional[int] = attr.ib(default=None,  validator=not_none_validator)
    trajectory_id: Optional[Union[int, str]] = attr.ib(default=None)


@registry.register_dataset(name="R2R-IE-CE-v1")
class R2RCIECEv1(Dataset):
    ...
    def from_json(
        ...
        for episode in deserialized["episodes"]:
            ...
            if episode.error_information is not None:
                episode.error_information = ErrorInformationData(**episode.error_information)
